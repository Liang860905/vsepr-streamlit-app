...
# 頁面配置與全局 CSS (置中、水平排列 radio、字體調整)
...
def show_vsepr_teardrop(domains, shape_name, show_angle_labels=True):
    # 若為 4 電子域，強制改為理想正四面體排列（保留原 type）
    if len(domains) == 4:
        R = 2.5
        s = R / math.sqrt(3)
        domains = [
...
# 頁面標題
st.markdown("<h1>VSEPR 模型</h1>", unsafe_allow_html=True)

# 選擇區塊
with st.container():
    st.markdown("<h2>選擇區</h2>", unsafe_allow_html=True)
    # 利用 st.radio 水平排列選項
...
# 模型顯示區塊
with st.container():
    st.markdown("<h2>模型顯示區</h2>", unsafe_allow_html=True)
    ...
    st.markdown(f"<p style='font-size:16px;'>{data['shape_name']}</p>", unsafe_allow_html=True)
