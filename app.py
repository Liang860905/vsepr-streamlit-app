...
def add_axes(view, axis_length=3.0):
    radius = 0.05
    view.addCylinder({'start': {'x': -axis_length, 'y': 0, 'z': 0},
                      'end':   {'x': axis_length, 'y': 0, 'z': 0},
                      'radius': radius, 'color': 'red'})
    view.addCylinder({'start': {'x': 0, 'y': -axis_length, 'z': 0},
                      'end':   {'x': 0, 'y': axis_length, 'z': 0},
                      'radius': radius, 'color': 'green'})
    view.addCylinder({'start': {'x': 0, 'y': 0, 'z': -axis_length},
...
def add_teardrop_lobe(view, x, y, z, color='lightblue', steps=20, include_ligand=True):
    ...
    if include_ligand:
        ...
        view.addSphere({'center': {'x': sphere1_center[0], 'y': sphere1_center[1], 'z': sphere1_center[2]},
                        'radius': 0.1, 'color': 'black', 'opacity': 1.0})
        view.addSphere({'center': {'x': sphere2_center[0], 'y': sphere2_center[1], 'z': sphere2_center[2]},
                        'radius': 0.1, 'color': 'black', 'opacity': 1.0})
...
def add_arc_between(view, v1, v2, segments=20, allow_180_label=False):
    ...
    for i in range(segments+1):
        ...
        point = (r*(u1[0]*math.cos(phi) + n_vec[0]*math.sin(phi)),
    ...
    for i in range(len(arc_points)-1):
        ...
        view.addCylinder({'start': {'x': p_start[0], 'y': p_start[1], 'z': p_start[2]},
                          'end':   {'x': p_end[0], 'y': p_end[1], 'z': p_end[2]},
                          'radius': 0.02,
                          'color': 'lightgray',
                          'opacity': 0.5})
    mid_phi = angle/2.0
    mid_point = (r*(u1[0]*math.cos(mid_phi) + n_vec[0]*math.sin(mid_phi)),
...
# 顯示標題，這裡只顯示 shape_name（不附加額外說明文字）
